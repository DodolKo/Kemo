  ...
  <AppWidget variant="compact" size="lg" :width="1" :height="1">
    <div class="gauge-wrapper">
      <svg class="gauge-svg" viewBox="0 0 200 200">
        ...
        <circle class="gauge-progress" cx="100" cy="100" r="85" fill="none" :stroke="getColor(temperature)" stroke-width="12" stroke-linecap="round" :stroke-dasharray="circumference" :stroke-dashoffset="dashOffset" transform="rotate(-90 100 100)" :style="{ filter: getCircleGlow(temperature) }" />
      </svg>
      <div class="gauge-content">
        <div class="gauge-icon">🌡️</div>
        <div class="gauge-value" :style="{ color: getColor(temperature), textShadow: getGlow(temperature) }">{{ temperature }}°</div>
...
const cpuUsage = ref(50) // Pour la corrélation
const radius = 85
const circumference = 2 * Math.PI * radius
let intervalId = null

const dashOffset = computed(() => {
  // Normaliser la température de 30-100°C vers 0-100%
  const normalized = ((temperature.value - 30) / 70) * 100
  return circumference * (1 - Math.max(0, Math.min(100, normalized)) / 100)
})
...
defineExpose({ start, stop, value: computed(() => temperature.value) })
</script>
