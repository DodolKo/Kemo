  ...
  <AppWidget variant="compact" size="lg" :width="1" :height="1">
    <div class="gauge-wrapper">
      <svg class="gauge-svg" viewBox="0 0 200 200">
        ...
        <circle class="gauge-progress" cx="100" cy="100" r="85" fill="none" :stroke="getColor(ramUsage)" stroke-width="12" stroke-linecap="round" :stroke-dasharray="circumference" :stroke-dashoffset="dashOffset" transform="rotate(-90 100 100)" :style="{ filter: getCircleGlow(ramUsage) }" />
      </svg>
      <div class="gauge-content">
        <div class="gauge-icon">💾</div>
        <div class="gauge-value" :style="{ color: getColor(ramUsage), textShadow: getGlow(ramUsage) }">{{ ramUsage }}%</div>
